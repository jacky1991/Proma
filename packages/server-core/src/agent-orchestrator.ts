...
import type { AgentRuntime, AgentSendInput, AgentMessage, AgentGenerateTitleInput, AgentProviderAdapter, AgentSessionMeta, CodexOAuthCredentials, TypedError, RetryAttempt, SDKMessage, SDKAssistantMessage, AgentStreamPayload, RewindSessionResult, ProviderType, AgentQueryInput } from '@proma/shared'
import {
...
import { injectBuiltinMcpServers, type BuiltinMcpDeps } from './builtin-mcp/registry'
import { injectChromeDevtoolsMcpServer } from './builtin-mcp/chrome-devtools'
import { isBuiltinMcpUserEnabled } from './builtin-mcp/settings'
import { buildPiBuiltinTools, type PiBuiltinToolDeps } from './adapters/pi-builtin-tools'
...
function normalizeAgentRuntime(_value: unknown): AgentRuntime {
  // M2 迭代 3：Claude runtime 已移除，Pi 为唯一 runtime
  return 'pi'
}
...
function resolveSDKCliPath(): string {
  ...
  if (getEnvProbe().isPackaged && binaryPath.includes('.asar')) {
 ...
 * Orchestrator 可选依赖注入
 *
 * Electron 端注入真实实现（automation / collaboration / web-search 工具）；
 * Server 端 M2 传空（不启用这些内置工具，M3 按需接入）。
 */
export interface OrchestratorDeps {
  /** Pi runtime 内置工具依赖（automation / collaboration / web-search） */
  piBuiltinToolDeps?: PiBuiltinToolDeps
  /** 内置 MCP server 依赖（automation / collaboration） */
  builtinMcpDeps?: BuiltinMcpDeps
}
...
export class AgentOrchestrator {
  ...
  /**
   * 会话归属登记表：sessionId → 归属 userId
   *
   * sendMessage 入口按 scope 登记，emitEvent 发射事件时随事件总线透传给 WS 推送层，
   * 用于 '*' 通配订阅的按用户过滤（M3 迭代 8）。会话归属不会跨用户变更，条目无需失效。
   */
  private sessionOwners = new Map<string, string>()

  constructor(adapter: AgentProviderAdapter, eventBus: AgentEventBus, deps: OrchestratorDeps = {}) {
   ...
   * 发射事件（携带会话归属用户 ID）
   *
   * 归属在 sendMessage 入口按 scope 登记（sessionOwners）。WS 推送层据此对 '*'
   * 通配订阅做按用户过滤：仅归属用户与管理员可收到会话事件。桌面单用户场景无
   * scope，ownerUserId 为 undefined。
   */
  private emitEvent(sessionId: string, payload: AgentStreamPayload): void {
    this.eventBus.emit(sessionId, payload, this.sessionOwners.get(sessionId))
  }
  ...
  private async buildSdkEnv(
    ...
    applyAgentSdkAuthEnv(sdkEnv, provider, apiKey, getPromaUserAgent(getEnvProbe().version))
    if (provider === 'minimax') {
  ...
  async sendMessage(input: AgentSendInput, callbacks: SessionCallbacks, scope?: UserScope): Promise<void> {
    ...
    // 登记会话归属：WS '*' 通配广播据此按用户过滤（见 emitEvent 与 apps/server ws.ts）
    if (scope) this.sessionOwners.set(sessionId, scope.userId)
    ...
    const persistInitialUserMessage = (): void => {
      ...
      this.persistUserMessage(sessionId, rawUserMessage ?? userMessage, Date.now(), scope)
    ...
    applyAgentSdkAuthEnv(process.env, channel.provider, apiKey, getPromaUserAgent(getEnvProbe().version))
    ...
    try {
      // Claude runtime 已移除，无需加载 SDK / 检查 binary
      const sdk = undefined
      ...
      // 9.5 Claude SDK 项目设置已移除（Pi runtime 无需 .claude/settings.json）
      ...
      if (initialPermissionMode === 'plan') {
        this.emitEvent(sessionId, { kind: 'proma_event', event: { type: 'enter_plan_mode', sessionId } })
      ...
      const handleExitPlanMode = (toolInput: Record<string, unknown>, signal: AbortSignal): Promise<ExitPlanPermissionResult> => {
        return exitPlanService.handleExitPlanMode(
          ...
          (request: ExitPlanModeRequest) => {
            this.emitEvent(sessionId, { kind: 'proma_event', event: { type: 'exit_plan_mode_request', request } })
          },
        )
      }
        ...
        if (toolName === 'AskUserQuestion') {
          return askUserService.handleAskUserQuestion(
            ...
            (request: AskUserRequest) => {
              this.emitEvent(sessionId, { kind: 'proma_event', event: { type: 'ask_user_request', request } })
            },
          )
        }
      ...
      const handleSessionId = (sdkSessionId: string, piSessionFile?: string): void => {
        ...
        if (isNewSessionId || needsPiSessionFile) {
          try {
            ...
            const latestSessionMeta = getAgentSessionMeta(sessionId, scope)
      ...
      const handleModelResolved = (model: string): void => {
        ...
        this.emitEvent(sessionId, { kind: 'proma_event', event: { type: 'model_resolved', model: resolvedModel } })
      }
      ...
      const queryOptions = agentRuntime === 'pi' ? {
        agentRuntime: 'pi',
        sessionId,
        prompt: finalPrompt,
        ...
        onPiEntryBindings: (bindings: Record<string, string>) => {
          ...
          if (latest?.agentRuntime !== agentRuntime) return
          updateAgentSessionMeta(sessionId, {
      ...
      for (let attempt = 1; attempt <= MAX_AUTO_RETRIES + 1; attempt++) {
        ...
        try {
          ...
          const queryIterable = this.adapter.query(queryOptions as AgentQueryInput)
          ...
          while (true) {
            ...
            if (awaitingBackgroundWake) {
              ...
              if (msg.type === 'assistant' || msg.type === 'user' || sub === 'task_started' || sub === 'task_progress') {
                ...
                this.emitEvent(sessionId, { kind: 'proma_event', event: { type: 'run_resumed', sessionId } })
              }
            }
            ...
            if (msg.type === 'assistant' && !isPartialMessage) {
              ...
              if (assistantMsg.error) {
                ...
                this.emitEvent(sessionId, { kind: 'sdk_message', message: errorSDKMsg })
            ...
            if (msg.type === 'result') {
              ...
              // 完整白名单见 sdk-error-utils.ts 的 CONTINUABLE_TERMINAL_REASONS。
            ...
            if (!shouldEmit) {
              ...
              this.emitEvent(sessionId, { kind: 'sdk_message', message: msg })
            }
          }
          ...
          if (!wasStoppedByUser && retryAttemptsScheduled > RETRY_VISIBILITY_THRESHOLD) {
            this.emitEvent(sessionId, { kind: 'proma_event', event: { type: 'retry', status: 'cleared' } })
   ...
   *
   * 供路由层回调事件（permission_resolved / ask_user_resolved 等）补归属：
   * 这些事件应推送给会话所有者与管理员，归属以 sendMessage 入口登记为准，
   * 而非响应请求的调用者（管理员也可能代答他人会话的权限请求）。
   */
