/**
 * Office 内联预览服务 —— DOCX/XLSX/PPTX → HTML
 *
 * 与 document-parser.ts 职责并列、不可互替：
 * - document-parser 提取**纯文本**（供附件上下文注入）
...
/** mammoth 动态导入的最小类型契约（与 document-parser 保持一致的 cast 风格） */
interface MammothHtmlModule {
  convertToHtml(input: { path?: string; buffer?: Buffer }): Promise<{ value: string }>
}

/** officeparser 动态导入的最小类型契约 */
...
export async function convertDocxToHtml(filePath: string, basePaths?: string[]): Promise<{ resolvedPath: string; html: string } | null> {
  ...
  try {
    ...
    const mammoth = await import('mammoth') as unknown as MammothHtmlModule
    const result = await mammoth.convertToHtml({ path: safePath })
    return { resolvedPath: safePath, html: result.value }
  } catch (err) {
    console.error('[office-preview] convertDocxToHtml failed:', err)
    return null
  }
}
...
export async function convertOfficeToHtml(filePath: string, basePaths?: string[]): Promise<OfficePreviewResult | null> {
  ...
  try {
    ...
    console.error('[office-preview] convertOfficeToHtml structured preview failed:', err)
    try {
