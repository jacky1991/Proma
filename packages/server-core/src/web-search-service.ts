...
import { getChatToolsConfig, getToolCredentials } from './chat-tool-config'
...
export function isWebSearchEnabledForAgent(): boolean {
  // 单次读配置，避免 getToolState + isWebSearchAvailable 各读一次
  const config = getChatToolsConfig()
  return (config.toolStates['web-search']?.enabled ?? false)
    && !!(config.toolCredentials['web-search']?.apiKey)
}
...
/** 将未知输入归一化为去空字符串数组；空则返回 undefined（供 Chat/Agent 工具参数共用） */
...
async function fetchWithTimeout(url: string, init: RequestInit, fetchFn: typeof globalThis.fetch, timeoutMs = DEFAULT_TIMEOUT_MS): Promise<Response> {
...
async function postTavily<T>(endpoint: string, apiKey: string, body: Record<string, unknown>, signal?: AbortSignal): Promise<T> {
  // 走应用代理链（与企业自部署/国内网络场景一致，与 chat-service 出站请求同源）
