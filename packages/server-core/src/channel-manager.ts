...
import { getCryptoPort, getEnvProbe } from './config'
import type {
  Channel,
  ChannelCreateInput,
  ChannelUpdateInput,
 ...
 * Codex 模型拉取注入点
 *
 * listCodexModels 依赖 Pi SDK 适配层（pi-model-registry → pi-agent-adapter → runtime-guards/env），
 * 属 orchestrator 运行时，不进 server-core。桌面端启动时调用 setCodexModelsFetcher 注入真实实现；
 * 服务端未注入时，fetchModels 的 codex 分支返回降级结果。
 */
type CodexModelsFetcher = () => Promise<{ id: string; name: string }[]>
let listCodexModelsFn: CodexModelsFetcher | undefined
export function setCodexModelsFetcher(fn: CodexModelsFetcher | undefined): void {
  listCodexModelsFn = fn
}
/** 连接测试 / 模型拉取的统一超时时间 */
...
async function testAnthropicCompatible(
  ...
  if (provider === 'kimi-coding' || provider === 'zhipu-coding' || provider === 'zhipu-coding-team') {
    ...
    headers['User-Agent'] = getPromaUserAgent(getEnvProbe().version)
...
async function testQwenTokenPlanMessages(
  ...
  const response = await fetchFn(url, withTimeout({
    ...
    headers: {
      ...
      'content-type': 'application/json',
      Authorization: `Bearer ${apiKey}`,
      'User-Agent': getPromaUserAgent(getEnvProbe().version),
    },
    body: JSON.stringify({
      model: modelId,
      max_tokens: 8,
...
export async function fetchModels(input: FetchModelsInput): Promise<FetchModelsResult> {
  ...
  try {
    switch (provider) {
      ...
      case 'openai-codex':
        if (provider === 'openai-codex') {
          // ChatGPT (Codex) 模型目录依赖 Pi SDK 适配层（orchestrator 运行时），不进 server-core；
          // 由桌面端启动时经 setCodexModelsFetcher 注入真实实现；未注入时降级返回。
          if (!listCodexModelsFn) {
            return { success: false, message: 'ChatGPT (Codex) 模型拉取未配置（需桌面端注入）', models: [] }
          }
          const codexModels = await listCodexModelsFn()
          return {
            success: true,
