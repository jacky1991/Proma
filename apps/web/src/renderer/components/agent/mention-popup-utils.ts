...
export function shouldSuppressEscTrigger(
  ...
  // startsWith 已蕴含 length >= 关系，无需单独判断长度。
  return trigger.text.startsWith(suppressed.text)
}
...
export function shouldClearEscSuppressionOnExit(
  ...
  // 触发符已不在 range（range 越界或该处文本不再以触发符开头）= 片段已删除，清除抑制。
  // 复用 isSuggestionTriggerPresent（同一「range 内触发符是否仍在」判定），避免双份边界逻辑漂移。
  return !isSuggestionTriggerPresent(editor, range, char)
}
