...
/** 判断 Base URL 是否偏离当前供应商的官方默认地址（第三方中转站）。移植自 main 88115353（#1253）。 */
...
export function ChannelForm({ channel, onSaved, onAgentEligibilityChange, onCancel }: ChannelFormProps): React.ReactElement {
  ...
  // 已确认风险的 Base URL（归一化后）；未确认的第三方地址会阻断 auto-save/测试/拉取/创建。
  ...
  const hasRequiredSecret = isZhipuTeamProvider
    ...
    : isCodexProvider
      ...
      : Boolean(apiKey.trim())
  // 第三方 Base URL 必须先经用户确认风险，确认前阻断所有写操作与外部请求。
  ...
  // 监听字段变化触发 auto-save
  React.useEffect(() => {
    scheduleAutoSave(models, name, provider, baseUrl, effectiveApiKey, enabled, requiresBaseUrlRiskAcknowledgement)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 第三方 Base URL 风险确认。移植自 main 88115353（#1253），去除商业版推广段落。 */}
      <AlertDialog
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-2">
                <p>该地址并非当前供应商的官方默认 Base URL。中转站可能篡改对话内容与模型响应，存在中间人攻击、凭据泄露与隐私风险。</p>
